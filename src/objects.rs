...
pub struct TextFileGroup {
    pub files: HashMap<Language, TextFile>,
    pub mapped_items: HashMap<u16, u16>,
...
pub const AUCTION_ITEMS_SETS: [u16; 16] = [
...
pub struct Bufs {
    ...
    pub main_buf: Vec<u8>,
    pub shops_buf: Vec<u8>,
    pub card_shops_buf: Vec<u8>,
    pub exp_buf: Vec<u8>,
    pub pack_select_buf: Vec<u8>,
    pub map_buf: Vec<u8>,
}
...
impl Executable {
    pub fn to_model_path(&self) -> &'static str {
        match self {
    ...
    pub fn to_stage_model_path(&self) -> &'static str {
        match self {
    ...
    while let Some(file_res) = pro_folder.next().await {
        ...
        let opt: Option<MapObject> = async move {
            ...
            let stage_encounter_set = buf[initsp_index..initp_end_index]
                ...
                .map(|idx| Pointer::from_instruction(&buf[initsp_index..initsp_index + idx * 4]))
                .map(|stage_encounters_address| {
            ...
            if let Some(idx) = stage_encounter_set {
                let looper = |index: i32, modder: i32| -> Vec<StageEncountersObject> {
                    let mut stage_encounters_objects = Vec::new();
                    let mut i = match modder {
                        -1 => -1,
                        _ => 0,
                    };

                    loop {
                        let mut reader = Cursor::new(&buf[(index + i * 0x1c) as usize..]);
                        ...
                        if !(stage_encounters_obj.areas[0].null()
                    ...
                    stage_encounters_objects
                };

                let mut objs_behind = looper(idx as i32, -1);
                objs_behind.reverse();

                stage_encounters_objects.extend(objs_behind.into_iter());

                let objs_forward = looper(idx as i32, 1);
...
pub async fn fix_lba(path: &PathBuf, objects: &mut Objects) -> anyhow::Result<()> {
    ...
    let mut new_main_executable = async_std::fs::File::create(format!(
        "extract/{}/{}",
        rom_name,
...
pub async fn write_objects(path: &PathBuf, objects: &mut Objects) -> anyhow::Result<()> {
    objects.enemy_stats.write_buf(&mut objects.bufs.stats_buf)?;
    objects.encounters.write_buf(&mut objects.bufs.map_buf)?;
    ...
    let mut new_map =
        File::create(format!("extract/{}/{}", rom_name, dmw3_consts::MAP_FILE)).await?;
    ...
    new_map.write_all(&objects.bufs.map_buf).await?;
